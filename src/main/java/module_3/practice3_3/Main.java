/**
 ...
 Задача 4.
 Создайте абстрактный класс Animal, который имеет абстрактные методы makeSound() и move(). Реализуйте этот класс в дочерних классах Dog, Cat и Bird, где каждый класс реализует свои звук и способ передвижения. Создайте метод Main и в нем проверьте работу классов.
 ...
 c)	Определите интерфейс Resizable, содержащий метод resize(int percent). Circle и Rectangle должны реализовать интерфейс Resizable. Метод resize() должен изменять размер круга или прямоугольника на указанный процент. Создайте метод Main и в нем проверьте работу классов.

 Задача 6.
 Создайте интерфейс Logger, содержащий метод log(String message). Реализуйте интерфейс в классах ConsoleLogger и FileLogger, которые выводят сообщение в консоль и записывают в файл соответственно. Создайте класс Application, который использует объекты ConsoleLogger и FileLogger для логирования сообщений. Создайте метод Main и в нем проверьте работу классов.
 ...
 Задача 8.
 Представьте, что вы разрабатываете систему управления умным домом. Вам необходимо создать иерархию классов для устройств в доме (например, светильники, кондиционеры, дверные замки) с возможностью управления ими через центральный контроллер.
 ...
 •	Реализуйте методы для управления устройствами, такие как turnOnAllDevices(), turnOffAllDevices(), controlDeviceById(), которые будут использовать полиморфизм для работы с различными типами устройств.
